...
def start(update, context):
    context.bot.send_sticker(update.effective_chat.id,'CAACAgIAAxkBAAIJjmM_ES4TZvE10MaaxXxYC2ruugakAAKLAQACK15TC6NhvGkkNINQKgQ')
...
def candy_count(update, context):
    ...
    if candy_count.isdigit():
        ...
        return PER_TURN
    elif candy_count=='/cancel':
          context.bot.send_sticker(update.effective_chat.id,'CAACAgIAAxkBAAIJiGM_EKSgaVj0DaEimZb0TnRl-PyHAAJiBQACP5XMCkCIeReG4EyMKgQ')
...
def player_turn(update, context):
    ...
    if player_turn.isdigit():
        ...
        if player_turn <= turn_count:
                ...
                if candy_count < 1:
                    context.bot.send_sticker(update.effective_chat.id,'CAACAgIAAxkBAAIJkWM_Ec39VVBBe8hpFWvJMaewxxzXAALoCgAC7qDxSLyDD2W6BiPFKgQ')
                    update.message.reply_text(
                        f'Игрок проиграл') 
...
def computer_turn(update, context):
    ...
    if candy_count < turn_count:
        turn_count = turn_count - (turn_count - candy_count)
        if candy_count>(turn_count+4) and turn_count>2:
            turn_count +=turn_count +random.Random.randint(-1,1)
    if candy_count > 1:
        candy_count -= turn_count-1
    else:
        candy_count -= turn_count
    if candy_count <1:
        context.bot.send_sticker(update.effective_chat.id,'CAACAgIAAxkBAAIJi2M_ESIR4d3mDhEgp_QJn98JGLhaAALRAQACFkJrCk0QzZjEFtlyKgQ')
        update.message.reply_text(
                        f'Бот проиграл')
        return ConversationHandler.END 
    context.user_data['candy_count'] = candy_count
