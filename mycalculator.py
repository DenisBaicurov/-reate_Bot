...
def choice(update, context):
    ...
    if user_choice == '/cancel':
         ...
         update.message.reply_text('Вы ввеличто-то не то')
         return CHOICE   


def rational_one(update, context):
    user = update.message.from_user
    logger.info("Введённое чисдо: %s: %s", user.first_name, update.message.text)
...
def operatons_complex(update, context):
    user = update.message.from_user
    logger.info(
        "Введённое число %s: %f / %f", user.first_name, update.message.text)
